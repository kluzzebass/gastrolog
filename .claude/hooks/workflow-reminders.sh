...
case "$command" in
  *"dcat close"*)
    echo '{"systemMessage":"STOP: NEVER close issues without explicit user approval. The workflow is: (1) set status to in_review, (2) ask the user to test, (3) ask if we can close it, (4) only run dcat close after user confirms, (5) upon closing, commit, merge and push. These commands are NOT auto-allowed — the user must click Allow in the permission prompt."}'
    ;;
