...
(file backings) and optionally --cloud-service (cloud-backed).`,
	...
	cmd.Flags().String("disk-free-warn", "", "free-space warn threshold on the vault's backing volume (e.g. 10GB); empty inherits the node default: max(10% of volume, 10GiB), capped at 25% of the volume")
	cmd.Flags().String("disk-free-floor", "", "free-space floor on the vault's backing volume (e.g. 3GB) — below it, admission for this vault is suspended; empty inherits the node default: max(3% of volume, 3GiB), capped at 10% of the volume")
