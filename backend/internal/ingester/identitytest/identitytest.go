...
// EventID-identity tests. Every ingester must produce IngesterMessages
// whose IngesterID matches the configured ID and whose IngestTS is
// non-zero — those are the two identity fields the ingestion pipeline
// needs to assemble a complete EventID. The test discipline catches
// regressions where a future ingester (or refactor) silently drops one
// of them.
...
func AssertHasIdentity(t *testing.T, msg ingestion.IngesterMessage, wantIngesterID string) {
	t.Helper()
	if msg.IngesterID == "" {
		t.Error("IngesterMessage.IngesterID is empty — ingester must set the configured ID on every emitted message")
	...
	if msg.IngestTS.IsZero() {
		t.Error("IngesterMessage.IngestTS is zero — ingester must stamp the receive time on every emitted message")
	}
}
