...
func (m *ingester) collectVaults() []orchestrator.IngestMessage {
	...
	for _, snap := range snapshots {
		raw := fmt.Sprintf(
			"record_count=%d chunk_count=%d sealed_chunks=%d data_bytes=%d enabled=%t vault_replica_spool_watermark=%d vault_ingest_high_watermark=%d vault_fence_high_watermark=%d",
			snap.RecordCount,
			snap.ChunkCount,
			snap.SealedChunks,
			snap.DataBytes,
