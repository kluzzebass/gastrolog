...
func TestReliability_Failover_LeaderDown_NewLeaderElected(t *testing.T) {
	...
	cmd := MarshalVaultChunkCommand(tierID, vaultctlfsm.MarshalCreateChunk(chunkIDWithPrefix(0x11), now, now, now))
...
func TestReliability_PipelinedApplies_SurviveLeaderKill(t *testing.T) {
	...
	for i := range byte(20) {
		...
		go func(cid chunk.ChunkID) {
			defer wg.Done()
			cmd := MarshalVaultChunkCommand(tierID, vaultctlfsm.MarshalCreateChunk(cid, now, now, now))
			n := h.nodes[oldLeader]
			n.mu.Lock()
			r := n.raft
			n.mu.Unlock()
