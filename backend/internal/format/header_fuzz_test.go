...
func FuzzDecode(f *testing.F) {
	...
	f.Add([]byte{Signature, TypeGLCB, 0x01, 0x00})
...
func FuzzDecodeAndValidate(f *testing.F) {
	f.Add([]byte{Signature, TypeGLCB, 0x01, 0x00}, byte(TypeGLCB), byte(0x01))
	f.Add([]byte{}, byte(0x00), byte(0x00))
	f.Add([]byte{0xff, 0xff, 0xff, 0xff}, byte(0x42), byte(0x01))
	f.Add([]byte{Signature, TypeRawLog, 0x02, FlagSealed}, byte(TypeRawLog), byte(0x02))
