...
func TestSourceIDStringFormat(t *testing.T) {
	id := NewSourceID()
	s := id.String()
	if len(s) != 36 {
		t.Fatalf("expected 36-char string, got %d: %q", len(s), s)
...
func TestParseSourceIDValid(t *testing.T) {
	known := "01234567-89ab-cdef-0123-456789abcdef"
	id, err := ParseSourceID(known)
	if err != nil {
		t.Fatalf("parse: %v", err)
	}
	if id.String() != known {
...
func TestSourceIDZero(t *testing.T) {
	zero := SourceID(uuid.UUID{})
	s := zero.String()
	parsed, err := ParseSourceID(s)
	if err != nil {
		t.Fatalf("parse zero: %v", err)
