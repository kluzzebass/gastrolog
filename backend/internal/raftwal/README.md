...
- **Drained** (zero live bytes): unlink outright. Before the `os.Remove`, a verification scan confirms no index reference survives into the segment — the live-bytes counter only nominates a segment for removal, the scan gates the removal itself. No rewrite I/O, but the scan walks every group's whole index once per unlink.
...
- **Reclamation**: Runs on the batchWriter, strictly after `notifyBatchWaiters` — waiters never wait on it. Unlinks every drained sealed segment it finds, then scavenges at most one nearly-drained segment. `Config.ScavengeMaxLiveBytes` bounds the rewrite in a pass and nothing else: the unlink loop is unbounded in count, and each unlink first scans the whole index to verify no reference survives. Callbacks (`OnReclaim`, `OnReclaimAnomaly`) are collected during the pass and invoked after `stateMu` is released, so a slow notification never blocks reads.
