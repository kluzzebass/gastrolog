...
- **Reclamation**: Runs on the batchWriter, strictly after `notifyBatchWaiters` — waiters never wait on it. Unlinks every drained sealed segment it finds, then scavenges at most one nearly-drained segment, so the inline rewrite cost of a single pass never exceeds `Config.ScavengeMaxLiveBytes`.
