...
// Filters are inlined as RouteConfig.Stages[].Match.Expression — there is no
// separate filter entity. The expression syntax (including the "*" catch-all
// and the empty-string match-nothing form) lives on
// RouteConfig.MatchStage.Expression.
...
type RetentionPolicyConfig struct {
	...
	// Resolved at use through the shared parser.
	MaxAge *string `json:"maxAge,omitempty"`

	// MaxSize is the vault's disk-claim bound, as a size expression
	// ("50GB"), and it means BOTH of these at once:
	//   - DRAIN: oldest sealed chunks past the bound are drained (per the
	//     vault's disposition). Unconditional — every set MaxSize drains,
	...
	//     up or is deferred. Gated by Refuse (default off): a MaxSize
	//     policy with no explicit Refuse:true only drains, it does not
	//     refuse. Resolved at the config→runtime boundary by
	//     refreshVaultDiskGuards: min over every REFUSE-ELIGIBLE attached
	//     policy's parsed MaxSize; no bound at all (refuse-only — a
	//     default must never destroy data) when no attached policy STATES
	//     a MaxSize. A stated-but-soft MaxSize does not engage the floor
	//     either; the operator's opt-out stands.
	MaxSize *string `json:"maxSize,omitempty"`
	...
	// policy states. DEFAULT OFF (operator decision:
	...
	// including a MaxSize policy with no explicit Refuse: it drains, but
	// does not refuse unless the operator opts in.
	//
	// MaxSize's refuse check is instantaneous (the disk guard's cap
	// machinery). MaxAge/MaxChunks refuse only once the
	...
	// opts in, there is no refuse bound at all for that kind. There is no
	// per-vault default to fall back to either: a vault with no stated
	// (or no refusing) size bound simply
...
// admission while violated. Unset (nil) defaults to false (operator
// decision: bounds are drain-first) — a policy must opt IN to
...
// check to reject empty configs at the admission boundary.
...
// unparseable value contributes nothing — it is a no-op condition. Resolved
// through the shared parser, like every other operator-authored quantity.
...
// RetentionRule binds a retention policy to a vault. A rule carries no
// action of its own: a fired retention event always streams the chunk's
// records through the routing engine with
// `source = retention-trigger(vault_id)` and always destroys the original
// chunk. The routing engine's verdict drives placement.
