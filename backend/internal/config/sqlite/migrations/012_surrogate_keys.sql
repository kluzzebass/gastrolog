-- Drop and recreate all tables with UUID PKs + name columns.
-- Not in production — no data migration needed.

DROP TABLE IF EXISTS stores;
DROP TABLE IF EXISTS filters;
DROP TABLE IF EXISTS rotation_policies;
DROP TABLE IF EXISTS retention_policies;
DROP TABLE IF EXISTS ingesters;
DROP TABLE IF EXISTS users;
DROP TABLE IF EXISTS tls_certificates;
DROP TABLE IF EXISTS settings;

-- UUID format: 8-4-4-4-12 lowercase hex with hyphens, 36 chars total.
-- length + negative GLOB is compact and catches non-hex/non-hyphen characters.
-- Example valid: 019c5d6d-020f-794e-bbff-5a80603aec79
