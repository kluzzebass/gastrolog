	...
	if c.cfg.Stats != nil {
		...
		for _, v := range c.cfg.Stats.VaultSnapshots() {
			stats.Vaults = append(stats.Vaults, &gastrologv1.VaultStats{
				...
				SpoolWatermark:           v.SpoolWatermark,
				FenceHighWatermark:       v.FenceHighWatermark,
				MaterializationWatermark: v.MaterializationWatermark,
				ConvergenceWatermark:     v.ConvergenceWatermark,
			})
		}
