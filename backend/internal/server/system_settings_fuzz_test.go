...
func FuzzValidateLookupNames(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, httpName, jsonName, mmdbName, csvName string) {
		...
		if httpName != "" {
			lc.HTTPLookups = []system.HTTPLookupConfig{{Name: httpName, URLTemplate: "http://example.com"}}
		}
		if jsonName != "" {
			lc.JSONFileLookups = []system.JSONFileLookupConfig{{Name: jsonName, FileID: "f"}}
		}
		if mmdbName != "" {
			lc.MMDBLookups = []system.MMDBLookupConfig{{Name: mmdbName}}
		}
		if csvName != "" {
			lc.CSVLookups = []system.CSVLookupConfig{{Name: csvName}}
		}
		// Must not panic on any input.
