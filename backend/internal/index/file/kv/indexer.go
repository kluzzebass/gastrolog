...
func (idx *Indexer) Build(ctx context.Context, chunkID chunk.ChunkID) error {
	...
	if result.capped {
		idx.logger.Warn("kv index capped due to hard cap",
			...
			"reason", result.capReason,
		)
		return idx.writeCappedIndexes(chunkID)
	}

	idx.computeCosts(result)
	keyEntries, valueEntries, kvEntries, stats := idx.admitAndBuild(result)
	...
	idx.logger.Debug("kv index built",
		...
		"dropped_keys", len(result.keyCandidates)-len(keyEntries),
		"dropped_values", len(result.valueCandidates)-len(valueEntries),
		"dropped_kv", len(result.kvCandidates)-len(kvEntries),
		"key_bytes", stats.keyBytes,
		"value_bytes", stats.valueBytes,
...
func (idx *Indexer) collectCandidates(ctx context.Context, chunkID chunk.ChunkID) (*kvCollectResult, error) {
	...
	for {
		...
		idx.processRecordPairs(result, valuesPerKey, pairs, ref.Pos)
	}
	_ = cursor.Close()
	return result, nil
}

func (idx *Indexer) processRecordPairs(result *kvCollectResult, valuesPerKey map[string]map[string]struct{}, pairs []tokenizer.KeyValue, pos uint64) {
	...
	for _, kv := range pairs {
		...
		if _, ok := result.keyCandidates[kv.Key]; !ok {
			if len(result.keyCandidates) >= MaxUniqueKeys {
				result.capped = true
				result.capReason = fmt.Sprintf("too many unique keys (limit %d)", MaxUniqueKeys)
				return
			}
		...
		if _, ok := valuesPerKey[kv.Key][kv.Value]; !ok {
			if len(valuesPerKey[kv.Key]) >= MaxValuesPerKey {
				result.capped = true
				result.capReason = fmt.Sprintf("too many values for key %q (limit %d)", kv.Key, MaxValuesPerKey)
				return
			}
		...
		if len(result.kvCandidates) >= MaxTotalEntries {
			result.capped = true
			result.capReason = fmt.Sprintf("too many total entries (limit %d)", MaxTotalEntries)
			return
		}
...
func admitWithinBudget(sorted []*kvCandidate, budget int64) ([]*kvCandidate, int64) {
...
func (idx *Indexer) admitAndBuild(result *kvCollectResult) ([]index.KVKeyIndexEntry, []index.KVValueIndexEntry, []index.KVIndexEntry, admitStats) {
	admittedKeys, keyBytes := admitWithinBudget(sortCandidates(result.keyCandidates), idx.kvBudget)
	admittedValues, valueBytes := admitWithinBudget(sortCandidates(result.valueCandidates), idx.kvBudget)
	admittedKV, kvBytes := admitWithinBudget(sortCandidates(result.kvCandidates), idx.kvBudget)
	...
	return keyEntries, valueEntries, kvEntries, admitStats{keyBytes: keyBytes, valueBytes: valueBytes, kvBytes: kvBytes}
}

func (idx *Indexer) writeAllIndexes(chunkID chunk.ChunkID, keyEntries []index.KVKeyIndexEntry, valueEntries []index.KVValueIndexEntry, kvEntries []index.KVIndexEntry) error {
