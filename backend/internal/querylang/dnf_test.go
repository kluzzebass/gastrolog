...
func TestToDNF(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantBranches: 1,
			wantString:   "(error AND warn)",
		},
		{
			name:         "two tokens OR",
			input:        "error OR warn",
			wantBranches: 2,
			wantString:   "error OR warn",
		},
		{
			name:         "NOT token",
			input:        "NOT error",
			wantBranches: 1,
			wantString:   "NOT error",
		},
		{
			name:         "token AND NOT token",
			input:        "error AND NOT debug",
			wantBranches: 1,
			wantString:   "(error AND NOT debug)",
		},
		{
			name:         "OR with NOT - distributes",
			input:        "(error OR warn) AND NOT debug",
			wantBranches: 2,
			wantString:   "(error AND NOT debug) OR (warn AND NOT debug)",
		},
		{
			name:         "complex AND OR",
			input:        "(a AND b) OR (c AND d)",
			wantBranches: 2,
			wantString:   "(a AND b) OR (c AND d)",
		},
		{
			name:         "three way OR",
			input:        "a OR b OR c",
			wantBranches: 3,
			wantString:   "a OR b OR c",
		},
		{
			name:         "double NOT",
			input:        "NOT NOT error",
		...
		},
		{
			...
			input:        "NOT (a AND b)",
			wantBranches: 2,
			wantString:   "NOT a OR NOT b",
		},
		{
			name:         "De Morgan OR",
			input:        "NOT (a OR b)",
			wantBranches: 1,
			wantString:   "(NOT a AND NOT b)",
		},
		{
			name:         "key=value",
			input:        "level=error",
		...
		},
		{
			...
			wantBranches: 1,
			wantString:   "(error AND level=warn)",
		},
		{
