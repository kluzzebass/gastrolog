...
export function ChunkTimeline({
  ...
  const { bars, ticks, laneCount } = (() => {
    ...
    const parsed = chunks
      .map((chunk) => {
        const start = (chunk.ingestStart ?? chunk.writeStart) ? instantToMs(protoToInstant((chunk.ingestStart ?? chunk.writeStart)!)) : 0;
        const end = (chunk.ingestEnd ?? chunk.writeEnd) ? instantToMs(protoToInstant((chunk.ingestEnd ?? chunk.writeEnd)!)) : start;
