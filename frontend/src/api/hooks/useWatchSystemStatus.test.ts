...
describe("applyStatusMessage", () => {
  // gastrolog-3cobq4 review: buildSystemStatus (backend) always populates
  // `storages`, including as an empty slice once the last storage is
  // removed. The old `msg.storages.length > 0` guard here dropped that
  // empty write, so the cache kept showing the deleted storage's card
  // forever — this pins the merge writing empty.
