...
/** Alarm shelving mutations. Servable from any node: the backend resolves
 *  every raiser of the alarm ID and fans the operation out — partial
 *  failures surface as errors naming the unreachable node, and the
 *  operations are idempotent, so retrying is always safe. Success
 *  refetches cluster status so the panel reflects the new state without
