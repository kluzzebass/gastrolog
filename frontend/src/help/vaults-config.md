...
- **Disk Free Warn** — free space on the vault's backing volume below which the disk-space alarm raises for this vault. An absolute size like `10GB`, or a percentage of the volume like `10%`. Leave empty to inherit the node default, `10%`.
- **Disk Free Floor** — free space below which the cluster stops accepting new records destined to this vault, on every node, until space frees. Records for other vaults keep flowing. A size like `3GB` or a percentage like `3%`; leave empty to inherit the node default, `3%`. The node defaults can be overridden with `GLOG_DISK_FREE_WARN` / `GLOG_DISK_FREE_FLOOR` in the same size-or-percentage form.
