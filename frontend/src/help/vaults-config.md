...
There is no vault-level size cap here anymore — the per-node disk-claim budget moved onto the retention policy. See [**Size Budget**](help:policy-retention) on the Retention Policies help for the cap-and-refuse mechanics, the min-wins resolution across a vault's attached policies, and the default floor that applies when no attached policy carries one.
