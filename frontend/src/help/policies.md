...
The size limit is a **soft limit** — it checks the projected size (current size plus the incoming record) before each append. If the projected size would exceed the limit, the chunk is sealed first and the record goes into a new chunk.

### Hard Limit Safety Net

Regardless of your configured policy, a built-in hard limit at **4 GB** is always enforced. This prevents corruption from 32-bit offset overflow in the file format. You cannot disable this — it fires automatically if the configured policy would let a chunk grow too large.

### Size Values

Size fields (`maxBytes`) accept values with suffixes: `B`, `KB`, `MB`, `GB` (case-insensitive). Examples: `64MB`, `1GB`, `512KB`. A bare number is treated as bytes.

### Duration Values

Duration fields (`maxAge`) accept Go duration syntax: `30s`, `5m`, `1h`, `24h`, `720h`. The age is measured from wall-clock time when the chunk was opened, not from the first record's timestamp.
...
- `30 0 * * * *` — every minute at second 30 (6-field)

Cron rotation only fires if the active chunk has at least one record — empty chunks are not sealed.
