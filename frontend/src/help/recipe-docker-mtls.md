...
Use the `filter` param to limit which containers are logged. It uses the same expression language as [search queries](help:query-language) and [vault filters](help:routing), applied to container metadata (`name`, `image`, `label.<key>`):
