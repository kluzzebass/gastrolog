...
Once a record has been ingested and digested, it is [filtered](help:routing) into one or more **stores** based on filter expressions. Each store appends the record to its active chunk.

Stores manage the full lifecycle of your data:

- [**Filtering**](help:routing) controls which records reach which stores
