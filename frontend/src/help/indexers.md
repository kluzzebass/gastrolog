...
After a chunk is sealed, GastroLog builds **inverted indexes** on its contents in the background — data structures that map search terms to record positions. Instead of scanning every record, the engine looks up which positions match and reads only those. You don't configure or manage indexes — they're built automatically.
...
**Record attributes** — The key-value pairs stored alongside each record (like `host=web-01` or `level=error`). These are indexed exactly as stored, so `level=error` queries are fast and precise.
...
If a KV index runs out of budget (too many distinct keys in one chunk), it's marked as **capped** and the engine falls back to scanning for those predicates. The Explain view shows when this happens.
